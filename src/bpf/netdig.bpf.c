...
SEC("kprobe/hook_l3_skb_start")
int kprobe__hook_l3_skb_srart(struct pt_regs *ctx) {
...
SEC("kprobe/hook_l3_skb_process")
int kprobe__hook_l3_skb_process(struct pt_regs *ctx) {
  struct sk_buff *skb = (struct sk_buff *)PT_REGS_PARM1(ctx);
...
SEC("kprobe/hook_l3_skb_end")
int kprobe__hook_l3_skb_end(struct pt_regs *ctx) {
  struct sk_buff *skb = (struct sk_buff *)PT_REGS_PARM1(ctx);
